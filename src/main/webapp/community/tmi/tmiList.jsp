...
<body>
	...
	<table class="tmiList">
			...
			<c:when test="${not empty tmiPageInfo.listCount }">
				<c:forEach var="tmiBoardList" items="${tmiBoardList }">
					<tr>
						...
						<td class="link" width="350" height="50"
							onclick="location.href='TmiDetail.co?idx=${tmiBoardList.idx }&pageNum=${tmiPageInfo.pageNum}&keyword=${param.keyword }'">
		...
		<!-- 버튼 창 -->
		<tr>
			<td id="plus"><i class='fas fa-plus-circle'
				style='position: static; font-size: 48px; color: #6C757D;'
				onclick="location.href='TmiWriteForm.co'" /></i></td>
		</tr>
	</table>
	...
	<div align="center">
		<form>
			<input class="search" type="text" placeholder="Search" name="keyword"
				value="${param.keyword }"> <input class="search"
				id="searchIcon" type="submit" value="검색">
		</form>
	</div>
