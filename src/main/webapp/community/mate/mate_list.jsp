	...
	<table class="mateList">
				...
				<c:forEach var="mate" items="${mateList }">
					<tr class="mateListTable">
						<td class="link" width="350" height="50"
							onclick="location.href='MateDetail.co?idx=${mate.idx }&pageNum=${pageInfo.pageNum}'">${mate.subject }</a></td>
						<td>${mate.nickname }</td>
		...
		<tr>
			<td id="plus"><i class='fas fa-plus-circle'
				...
				onclick="location.href='MateWriteForm.co'" /></i></td>
		</tr>
	</table>
		...
		<c:choose>
			<c:when test="${pageInfo.pageNum > 1}">
				<input type="button" value="이전"
					onclick="location.href='MateList.co?pageNum=${pageInfo.pageNum - 1}'"
					class="before_next">
			</c:when>
			<c:otherwise>
